...
function [ DELTA, FRS, ACCX_Index, ACCY_Index, ACCZ_Index ] = ...
    FEATURES( BP_PPG1, BP_PPG2, BP_ACCX, BP_ACCY, BP_ACCZ, ACCX_Index, ACCY_Index, ACCZ_Index, Freq_Ind, Nw, fs, N, NO_FRS)
T = 0.3;
delta = 18;
...
[ M1 , CP1, ~, P1] = findpeaks(Normalized_Spec1, 'MinPeakHeight', T);
...
FRS(4, i) = M(i) - M_Star(1);

[FRS(5, i), index1] = max(Normalized_Spec1 ( DELTA(i)-delta : DELTA(i)+delta ));
FRS(6, i) = abs(DELTA(i) - (index1+DELTA(i)-delta-1));
...
[FRS(11, i), ~] = max(Normalized_Spec2 ( 4*DELTA(i)-4*delta : 4*DELTA(i)+4*delta ));
% FRS(24, i) = abs(DELTA(i) - (index4+4*DELTA(i)-4*delta-1)/4);

Scaled_PPG1 = BP_PPG1 / max(BP_PPG1);
Scaled_PPG2 = BP_PPG2 / max(BP_PPG2);
FRS(12, i) = abs(HR_gamma(i) - length(findpeaks(Scaled_PPG1, 'MinPeakHeight', 0, 'MinPeakDistance', 1000/((Freq_Ind_Prev+30)*fs/N*8))));
FRS(13, i) = abs(HR_gamma(i) - length(findpeaks(Scaled_PPG2, 'MinPeakHeight', 0, 'MinPeakDistance', 1000/((Freq_Ind_Prev+30)*fs/N*8))));
FRS(14, i) = abs(HR_gamma(i) - length(findpeaks(Scaled_PPG1, 'MinPeakHeight', 0.2, 'MinPeakDistance', 1000/((Freq_Ind_Prev+30)*fs/N*8))));
FRS(15, i) = abs(HR_gamma(i) - length(findpeaks(Scaled_PPG2, 'MinPeakHeight', 0.2, 'MinPeakDistance', 1000/((Freq_Ind_Prev+30)*fs/N*8))));
FRS(16, i) = abs(HR_gamma(i) - length(findpeaks(Scaled_PPG1, 'MinPeakHeight', 0.4, 'MinPeakDistance', 1000/((Freq_Ind_Prev+30)*fs/N*8))));
FRS(17, i) = abs(HR_gamma(i) - length(findpeaks(Scaled_PPG2, 'MinPeakHeight', 0.4, 'MinPeakDistance', 1000/((Freq_Ind_Prev+30)*fs/N*8))));
